...
#################################################### Pretraining ####################################################
python transformers/examples/xla_spawn.py --num_cores 8 transformers/examples/language-modeling/run_alignedmlm_synthetic_transitive.py --warmup_steps 10000 --learning_rate 1e-4 --save_steps -1 --max_seq_length 512 --logging_steps 50 --overwrite_output_dir --model_type roberta --config_name config/en/roberta_8/${config_name} --tokenizer_name config/en/roberta_8/ --do_train --do_eval --max_steps 500000 --per_device_train_batch_size 16 --per_device_eval_batch_size 16 --train_file ../../../bucket/pretrain_data/en/train.txt --transitive_file ../../../bucket/pretrain_data/en/train.txt --validation_file ../../../bucket/pretrain_data/en/valid.txt --output_dir ${pretrain_model} --run_name ${base_dir} --one_to_one_mapping --word_modification replace --alignment_loss_weight 10 --bilingual_rate 0.25
